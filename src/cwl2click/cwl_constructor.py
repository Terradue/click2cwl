...
class CwlCreator:
    ...
    def create_clt(self, ctx, docker, requirements, env):
        ...
        for i in range(len(self.command_names)):
            if "enum" in self.get_input_type(self.option_list[i]) and self.option_list[i].required == False:
                self.set_clt_inputs({'inp' + str(i + 1): {
                    'inputBinding': {'position': i + 1, 'prefix': "--" + str(self.command_names[i])},
                    'type': self.get_input_type(self.option_list[i]),'symbols':self.option_list[i].type.choices[:]}})

            elif "enum" in self.get_input_type(self.option_list[i]) and self.option_list[i].required :
                 self.set_clt_inputs({'inp' + str(i + 1): {
                    'inputBinding': {'position': i + 1, 'prefix': "--" + str(self.command_names[i])},
                    'type':['null',{'type':self.get_input_type(self.option_list[i])},{'symbols':self.option_list[i].type.choices[:]}]}})
            else:
                self.set_clt_inputs({'inp' + str(i + 1): {
                    'inputBinding': {'position': i + 1, 'prefix': "--" + str(self.command_names[i])},
            ...
            if self.get_input_type(self.option_list[i])!= 'enum' :
                self._workflow_class['inputs'][self.command_names[i]] = \
                    {'doc': str(self.command_names[i]) + " doc",
                     ...
                     'type': str(self.get_input_type(self.option_list[self.command_names.index(self.command_names[i])]))}
        ...
        with open(self.ctx.command_path + '.yml', 'w') as file:
            for i in range(len(self.command_names)):
                if "File" in self.get_input_type(self.option_list[i]):
                    yaml.dump({self.command_names[i]:{'class':'File',
                    'path':self.ctx.params[self.command_names[i]].name}},file)
                else:
                    yaml.dump({self.command_names[i]:self.ctx.params[self.command_names[i]]}, file) 
            file.close()
